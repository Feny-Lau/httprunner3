...
absolute_http_url_regexp = re.compile(r"^https?://", re.I)  # 匹配字符串的开头,忽略大小写, 匹配http或者https

# use $$ to escape $ notation
dolloar_regex_compile = re.compile(r"\$\$")     # 匹配$$符号
# variable notation, e.g. ${var} or $var
# variable should start with a-zA-Z_
variable_regex_compile = re.compile(r"\$\{([a-zA-Z_]\w*)\}|\$([a-zA-Z_]\w*)")           # 匹配测试数据中的变量
# function notation, e.g. ${func1($var_1, $var_3)}
function_regex_compile = re.compile(r"\$\{([a-zA-Z_]\w*)\(([\$\w\.\-/\s=,]*)\)\}")      # 匹配测试数据中的方法
...
def regex_findall_variables(raw_string: Text) -> List[Text]:
    """ extract all variable names from content, which is in format $variable       # 匹配String中所有变量值, 并返回一个变量名组成的列表
    ...
    """
    try:
        match_start_position = raw_string.index("$", 0)     # 获取$ 所在位置
    except ValueError:
        return []

    vars_list = []
    while match_start_position < len(raw_string):           # 循环获取语句内所有$ 标记, 对$$进行转义,  对$区分 变量以及函数

        # Notice: notation priority
        # $$ > $var

        # search $$
        dollar_match = dolloar_regex_compile.match(raw_string, match_start_position)    # 实现$$ 转义
        if dollar_match:
        ...
        if var_match:
            var_name = var_match.group(1) or var_match.group(2)     # 第一个有意义的参数赋值给var_name
            vars_list.append(var_name)
            match_start_position = var_match.end()              # 记录找到变量的index ,再接着下一个index查找
            continue

        curr_position = match_start_position
        try:
            # find next $ location
            match_start_position = raw_string.index("$", curr_position + 1)     # 查找不到直接break
        except ValueError:
            # break while loop
            break
...
def extract_variables(content: Any) -> Set:
    """ extract all variables in content recursively.                将所有的集合拆箱, 子元素放在一个set中返回
    """
...
def parse_function_params(params: Text) -> Dict:
    """ parse function params to args and kwargs.       通过对字符串处理, 解析函数参数
...
def get_mapping_variable(
    ...
    """ get variable from variables_mapping.            # !!!!  解析variable : 在变量mapping中匹配变量名对应参数
    ...
    try:
        ...
        raise exceptions.VariableNotFound(
            f"{variable_name} not found in {variables_mapping}"         # 找不到就报not found error
        )
...
def get_mapping_function(
    ...
    """ get function from functions_mapping,
        if not found, then try to check if builtin function.        #  通过函数名到函数lsit匹配函数
    ...
    if function_name in functions_mapping:
        return functions_mapping[function_name]                 # 返回函数对应数据

    elif function_name in ["parameterize", "P"]:                # csv文件自动读取,并转化为tuple组成的list, 交给pytest的parametrized装饰器使用
        return loader.load_csv_file

    elif function_name in ["environ", "ENV"]:                   # .env环境变量匹配
        return utils.get_os_environ

    elif function_name in ["multipart_encoder", "multipart_content_type"]:      # 加载测试用例相关? 没用过
    ...
    try:
        ...
        built_in_functions = loader.load_builtin_functions()            # 匹配Httprunner内置方法, 没用过
...
def parse_string(
    ...
    """ parse string content with variables and functions mapping.          #!!!! 匹配文本中的变量名和函数名, 并替换为参数和运算结果
    ...
    try:
        ...
        parsed_string = raw_string[0:match_start_position]          # 解析$ 所在位置
    except ValueError:
...
def parse_variables_mapping(
    ...
    parsed_variables: VariablesMapping = {}         # 创造一个空的VariablesMapping (key是text的字典类型)

    while len(parsed_variables) != len(variables_mapping):
        for var_name in variables_mapping:

            if var_name in parsed_variables:            # 如果parsed_variables 已经有了变量名, 则跳过此次循环
                continue

            var_value = variables_mapping[var_name]
            variables = extract_variables(var_value)    # 如果没有,则将value的string都提出来
            ...
            if var_name in variables:
                ...
                raise exceptions.VariableNotFound(var_name)     # 变量值 引用变量名检查

            # check if reference variable not in variables_mapping
            not_defined_variables = [
                v_name for v_name in variables if v_name not in variables_mapping   # 检查variables中是否都在variables_mapping中
            ]
            if not_defined_variables:
                # e.g. {"varA": "123$varB", "varB": "456$varC"}
                # e.g. {"varC": "${sum_two($a, $b)}"}
                raise exceptions.VariableNotFound(not_defined_variables)        # 防止引用调用失败
